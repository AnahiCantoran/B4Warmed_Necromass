...
decompdata <- read.csv("/Users/anahicantoran/Desktop/B4W_NecroCore/Files/B4W_NecroCore_Map_CUT.csv", header = T) #Cut file has only ambC_ambR x 3.3C_redR treatments
...
```{r}
AsMod <- read.csv("/Users/anahicantoran/Desktop/B4W_NecroCore/FitbySampledata/Output2/AsymptoticModel.csv", header = T)
...
# Change the order of the treatment factor
...
k_plot <- ggplot(AsMod, aes(x=Treatment, y = kf, fill= Treatment)) +
 stat_boxplot(geom = "errorbar", width=0.2, position = position_dodge(1)) +
  geom_boxplot(position = position_dodge(1), outlier.shape = NA, alpha =0.9) + 
  ...
  scale_fill_manual(values = c("3.3C_redR"='#9A012C', "ambC_ambR"= '#4992AB')) +
  theme_bw() + 
...
anova_asmod <- aov(kf ~ Treatment + (1|Block), data = AsMod)
summary(anova_asmod)

#check homogeneity and normality 
check_homogeneity(anova_asmod)
is_norm <- check_normality(anova_asmod)
plot(is_norm)
...
## ANOVA ##
anova_asmod <- aov(A ~ Treatment + (1|Block), data = AsMod)
summary(anova_asmod)
...
k_A_plots_V <- as_ggplot(plot) +                                # transform to a ggplot
  draw_plot_label(label = c("Trt: ***"), size = 10, fontface = "plain",
                  y = c(0.95), x = c(0.35)) # Add labels
k_A_plots_V

#ggsave(file="Figures/Asymp_k_A_bytrt.svg",width = 6, height = 3, dpi = 300)
...
Meta <- read.csv("/Users/anahicantoran/Desktop/B4W_NecroCore/Files/B4W_NecroCore_Map.csv", header = T)
...
asmym.hb.plot <- ggplot(Meta_ctrl_trt, aes(Day, Mass_prop_rem, colour = Treatment)) + 
  ...
  scale_colour_manual(breaks = c( "3.3C_redR", "ambC_ambR"),
                      values=c('#9A012C','#4992AB' )) +
  scale_y_continuous(labels = function(x) paste0(x*100, "%")) +
  xlab("Day")+
  ylab("Necromass remaining") +
  theme_bw() + 
  theme(legend.position = c(.70, .85), legend.background = element_blank()) 

asmym.hb.plot 


#ggsave(file="Figures/Asympotic_trt_ctrl.svg", width = 8, height = 6, dpi = 300)
