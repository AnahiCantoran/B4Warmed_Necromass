...
# Read Files
```{r}
ClimateData <- read.csv(file = "/Users/anahicantoran/Desktop/B4W_NecroCore/Files/B4W_Climate_2023.csv", header = T)
...
ggplot(Field_VWC_week, aes(x=week, y=avgWVC_Wk, color = Treatment)) + geom_point()
```
...
```{r}
VWCData <- read.csv(file = "/Users/anahicantoran/Desktop/B4W_NecroCore/Files/Field_VWC.csv", header = T)
TempData <- read.csv(file = "/Users/anahicantoran/Desktop/B4W_NecroCore/Files/Field_Temp.csv", header = T)
...
ggplot(Fieldexp, aes(x=as.factor(week_exp), y=avgWVC, fill= Treatment)) + 
  geom_boxplot() +
  scale_fill_manual(values = c("3.3C_redR"='#B8062C', "ambC_ambR"= '#4992AB')) +
...
#ggsave(file="Figures/Field_VWC.svg",width = 10, height = 6, dpi = 300)
...
#ggsave(file="Figures/Field_Temp.svg",width = 10, height = 6, dpi = 300)

#Futher asthetics were developed in inkscape
```
